...
const collectEmployees = function () {
  ...
  // Loop to add employees and prompt user for input
  while (addEmployee === true) {
    let firstName = prompt("Enter first name:");
    let lastName = prompt("Enter last name:");
    let salaryNum = parseInt(prompt("Enter salary:"));
...
const displayAverageSalary = function (employeesArray) {
  ...
  let totalSalaryArray = employeesArray.map((a) => a.salary);
  let totalSalary = totalSalaryArray.reduce(
    (accumulator, salary) => accumulator + salary,
    0
  );

  // Calculate the average salary and convert to currency
...
const getRandomEmployee = function (employeesArray) {
  ...
  let numberOfEmployees = employeesArray.length;
  let randomEmployee = employeesArray[Math.floor(Math.random() * numberOfEmployees)];
